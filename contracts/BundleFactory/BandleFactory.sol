...
contract BundleFactory is IERC721Receiver {
    ...
    constructor(address _nftContract,address _BundleContract, address _proxyProvider) {
    ...
    function validateToken(address user, uint256 poolId) internal view returns (bool valid) {
    ...
    function build() external {
        ...
        for (uint256 i = 0; i < bundles.length; i++) {
            lockDealNFT.safeTransferFrom(address(this), address(proxyProvider), bundles[i]);
            params[i] = bundles[i]; 
            lockDealNFT.safeTransferFrom(address(this), address(lockDealBundleProvider), firstId + i);          
        }
        address token = lockDealNFT.getTokenAddress(bundles[0]);
        uint256 poolId = lockDealNFT.mint(msg.sender, token, msg.sender, 0, lockDealBundleProvider);
        lockDealBundleProvider._registerPool(poolId, msg.sender, token, params);
    }

    function extract(uint256 poolid) external {
        require(lockDealNFT.ownerOf(poolid) == address(this), "not in contract");
        require(isOwner(msg.sender, poolid), "not owner");
        lockDealNFT.safeTransferFrom(address(this), msg.sender, poolid);
    }

    function isOwner(address user, uint256 poolId) internal view returns (bool valid) {
        uint256[] memory bundles = userToBundles[msg.sender];
        require(bundles.length > 0, "no bundles");
        for (uint256 i = 0; i < bundles.length; i++) {
            if (bundles[i] == poolId) return true;
        }      
    }
}