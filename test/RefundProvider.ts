...
import { CollectorProvider } from "../typechain-types/contracts/CollectorProvider";
...
describe("Refund Provider", function () {
    ...
    let collectorProvider: CollectorProvider    
    let halfTime: number
    ...
    let BUSD: string
    let params: [BigNumber, number, number, BigNumber, number, BigNumber]
    ...
    before(async () => {
        ...
        collateralProvider = await deployed("CollateralProvider", lockDealNFT.address)
        collectorProvider = await deployed("CollectorProvider", lockProvider.address)
        refundProvider = await deployed("RefundProvider", lockDealNFT.address, collateralProvider.address, collectorProvider.address)
        ...
        await lockDealNFT.setApprovedProvider(collectorProvider.address, true)
    })
    ...
    beforeEach(async () => {
        ...
        params = [amount, startTime, finishTime, ethers.utils.parseEther('0.5'), (startTime + halfTime), rate]
    ...
    describe("Pool Creation", async () => {
        ...
        it("should return currect token data after creation", async () => {
        ...
        it("should return main coin data after creation", async () => {
            ...
            expect(poolData.params[0]).to.equal(ethers.utils.parseEther('0.5'))
            expect(poolData.params[1]).to.equal((startTime + halfTime))
        })

        it("check main coin collector data after creation", async () => {
            const poolData = await lockDealNFT.getData(poolId + 3)
            expect(poolData.provider).to.equal(collectorProvider.address)
            expect(poolData.poolInfo).to.deep.equal([poolId + 3, collateralProvider.address, constants.AddressZero])
        })

        it("check token collector data after creation", async () => {
            const poolData = await lockDealNFT.getData(poolId + 4)
            expect(poolData.provider).to.equal(collectorProvider.address)
            expect(poolData.poolInfo).to.deep.equal([poolId + 4, collateralProvider.address, constants.AddressZero])
        })
    })
    ...
    describe("Withdraw Pool", async () => {
        ...
        it("should deposit main coins after user withdraw", async () => {
            await time.setNextBlockTimestamp(finishTime)
            await lockDealNFT.withdraw(poolId)

            let collectorMainCoinPoolId =  poolId + 3
            const poolData = await lockDealNFT.getData(collectorMainCoinPoolId)
            expect(poolData.provider).to.equal(collectorProvider.address)
            expect(poolData.poolInfo).to.deep.equal([collectorMainCoinPoolId, collateralProvider.address, constants.AddressZero])
            expect(poolData.params[0]).to.equal(ethers.utils.parseEther('0.5'))
        })
    ...
    describe("Refund Pool", async () => {
        it("the user receives the main coins without", async () => {
            ...
            poolId += 5
            // const poolData = await lockDealNFT.getData(poolId)
            // expect(poolData.poolInfo).to.deep.equal([poolId, receiver.address, BUSD])
            // expect(poolData.params[0]).to.equal(amount / 2)
        })
        ...
        it("user withdraw the main coins", async () => {
            ...
            expect(withdrawnAmount).to.equal(ethers.utils.parseEther('0.5'))
