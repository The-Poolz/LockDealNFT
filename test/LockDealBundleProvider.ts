...
describe("Lock Deal Bundle Provider", function () {
    ...
    describe("Lock Deal Bundle Withdraw", () => {
        it("should withdraw all tokens before the startTime", async () => {
            ...
            const amount = await bundleProvider.getTotalRemainingAmount(bundlePoolId)
            await lockDealNFT.connect(receiver)["safeTransferFrom(address,address,uint256)"](receiver.address, lockDealNFT.address, bundlePoolId) // from DealProvider
            const amountAfterWithdraw = await bundleProvider.getTotalRemainingAmount(bundlePoolId)
            // expect(amountAfterWithdraw).to.equal(amount)
            // expect(amount).to.equal(amount.mul(3))
        })
